            ...
            for gen_it in range(generations):
                ...
                for sol_it in range(len(seed_exp)):
                    ...
                    for iterator in range(children_number):
                        ...
                        parents_index = np.random.choice([i for i in range(mu_it)], size=2, replace=False)

                        x_it = np.mean([x_parents[parents_index[0]], x_parents[parents_index[1]]])
                        y_it = np.mean([y_parents[parents_index[0]], y_parents[parents_index[1]]])
                        sigma_x_new = np.mean([sigma_x_parents[parents_index[0]], sigma_x_parents[parents_index[1]]])
                        sigma_y_new = np.mean([sigma_y_parents[parents_index[0]], sigma_y_parents[parents_index[1]]])
                ...
                sigma_x_parents = {j: sigma_x_total[i] for i,j in list(zip(list(z_total.keys())[:mu], [k for k in range(mu)]))}
                sigma_y_parents = {j: sigma_y_total[i] for i, j in list(zip(list(z_total.keys())[:mu], [k for k in range(mu)]))}
...
df_z.to_excel('z_values_p1.xlsx', index=False)
df_sigma.to_excel('sigma_values_p1.xlsx', index=False)
df_position.to_excel('position_p1.xlsx', index=False)