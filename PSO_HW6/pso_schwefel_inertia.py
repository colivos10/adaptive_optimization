...
for seed_it in range(10):
    ...
    while iterations_counter <= number_iterations:
        ...
        for i in range(pop_size):
            ...
            # For w
            p_velocity_cand_w = w_inertia * p_velocity[i][0] + np.random.uniform(0, phi_1) * (
                    p_position[i][0] - p_vector[i][0]) + np.random.uniform(0, phi_2) * (best_particle[0] - p_vector[i][0])

            while p_velocity_cand_w <= v_min or p_velocity_cand_w >= v_max:
                counter_phi += 1
                p_velocity_cand_w = w_inertia * p_velocity[i][2] + np.random.uniform(0, phi_1) * (
            ...
            # For z
            p_velocity_cand_z = w_inertia * p_velocity[i][3] + np.random.uniform(0, phi_1) * (
...
df_fitness_objectives.to_excel('PSO_HW6/results/fitness_total_v5_sch.xlsx', index = False)
df_best_objective.to_excel('PSO_HW6/results/best_objectives_v5_sch.xlsx', index = False)