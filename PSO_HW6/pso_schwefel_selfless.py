...
for seed_it in range(10):
    ...
    while iterations_counter <= number_iterations:
        ...
        second_best_index = (-p_position_fitness).argsort()[:pop_size][-2]

        best_particle = p_position[best_particle_index].copy()
        second_best_particle = p_position[second_best_index].copy()
        ...
        for i in range(pop_size):
            ...
            if i == best_particle_index:
                ...
                p_velocity_cand_x = p_velocity[i][0] + np.random.uniform(0, phi_1) * (
                        p_position[i][0] - p_vector[i][0]) + np.random.uniform(0, phi_2) * (second_best_particle[0] - p_vector[i][0])

                while p_velocity_cand_x <= v_min or p_velocity_cand_x >= v_max:
                    counter_phi += 1
                    p_velocity_cand_x = p_velocity[i][0] + np.random.uniform(0, phi_1) * (
                            p_position[i][0] - p_vector[i][0]) + np.random.uniform(0, phi_2) * (
                                                    second_best_particle[0] - p_vector[i][0])
                ...
                p_velocity_cand_y = p_velocity[i][1] + np.random.uniform(0, phi_1) * (
                        p_position[i][1] - p_vector[i][1]) + np.random.uniform(0, phi_2) * (second_best_particle[1] - p_vector[i][1])

                while p_velocity_cand_y <= v_min or p_velocity_cand_y >= v_max:
                    counter_phi += 1
                    p_velocity_cand_y = p_velocity[i][1] + np.random.uniform(0, phi_1) * (
                            p_position[i][1] - p_vector[i][1]) + np.random.uniform(0, phi_2) * (
                                                   second_best_particle[1] - p_vector[i][1])
                ...
                while p_velocity_cand_w <= v_min or p_velocity_cand_w >= v_max:
                    ...
                    p_velocity_cand_w = p_velocity[i][2] + np.random.uniform(0, phi_1) * (
                            p_position[i][2] - p_vector[i][2]) + np.random.uniform(0, phi_2) * (
                                                    second_best_particle[2] - p_vector[i][2])
                ...
                p_velocity_cand_z = p_velocity[i][3] + np.random.uniform(0, phi_1) * (
                        p_position[i][3] - p_vector[i][3]) + np.random.uniform(0, phi_2) * (second_best_particle[3] - p_vector[i][3])

                while p_velocity_cand_z <= v_min or p_velocity_cand_z >= v_max:
                    counter_phi += 1
                    p_velocity_cand_z = p_velocity[i][3] + np.random.uniform(0, phi_1) * (
                            p_position[i][3] - p_vector[i][3]) + np.random.uniform(0, phi_2) * (
                                                    second_best_particle[3] - p_vector[i][3])
...
df_fitness_objectives.to_excel('PSO_HW6/results/fitness_total_v4_sch.xlsx', index = False)
df_best_objective.to_excel('PSO_HW6/results/best_objectives_v4_sch.xlsx', index = False)