...
def z_function(x, y):
    z = (4 - 2.1 * x ** 2 + x ** 4 / 3) ** 2 + x * y + (-4 + 4 * y ** 2) * y ** 2
...
for i in range(number_iterations):
    for j in range(number_moves):
        ...
        # Get x next solution
        while x_temp > x_upper_bound and x_temp < x_lower_bound:
            x_temp = uniform_movement_operator(x_next, x_lower_bound, x_upper_bound)

        # Get y next solution
        while y_temp > y_upper_bound and y_temp < y_lower_bound:
        ...
        if z_temp <= z_next:
            x_next = x_temp
            y_next = y_temp
        elif np.random.random <= np.exp(-(z_temp - z_next)/ temperature_0):
            x_next = x_temp
            y_next = y_temp
